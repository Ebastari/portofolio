...
import { mailtoUrl, openEmail } from "./contact";
...
export default function MorphScroll() {
  ...
  useLayoutEffect(() => {
    ...
    // Elements are created up-front (drawAt indexes into them) but stay src-less
    // until requested — see the staged preload below.
    const frames: HTMLImageElement[] = LEVEL_FRAMES.map(() => {
      const im = new Image();
      im.decoding = "async";
      return im;
    });
    ...
    // Staged preload. The 304 frames are ~24 KB each (≈7 MB total), so weight isn't
    // the problem — issuing all 304 requests at once is: they saturate the connection
    // pool and delay the hero plate, which is what the visitor actually sees first.
    // So only the first REVEAL_AT go out on the critical path; the remaining ~270 are
    // queued at low priority once the stage has opened.
    ...
    function bump() {
      loadedCount++;
      if (loadedCount <= REVEAL_AT) setLoadProgress(Math.min(1, loadedCount / REVEAL_AT));
      if (lastF < 0 && imgReady(frames[0])) drawAt(0);
      if (loadedCount >= REVEAL_AT) reveal();
    }

    const startLoad = (i: number, priority?: "low") => {
      const im = frames[i];
      if (im.src) return;                       // already requested
      im.addEventListener("load", bump, { once: true });
      im.addEventListener("error", bump, { once: true });
      if (priority) im.fetchPriority = priority;
      im.src = LEVEL_FRAMES[i];
    };
    ...
    function reveal() {
      ...
      revealed = true;
      setAssetsReady(true);
      for (let i = REVEAL_AT; i < FRAME_COUNT; i++) startLoad(i, "low");
    }

    for (let i = 0; i < REVEAL_AT; i++) startLoad(i);
...
// ─── Icons ────────────────────────────────────────────────────────────────────
// Shared by WelcomeActions (slide 0) and PersistentCTA (flythrough onward) so both
// bars speak one visual language and no SVG is written twice.
...
const IconMail = ({ size = 16 }: { size?: number }) => (
  <svg viewBox="0 0 24 24" width={size} height={size} {...svgStroke} aria-hidden="true"><rect x="3" y="5" width="18" height="14" rx="2"/><path d="m3 7 9 6 9-6"/></svg>
);

const IconPhone = ({ size = 15 }: { size?: number }) => (
  <svg viewBox="0 0 24 24" width={size} height={size} {...svgStroke} aria-hidden="true"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72c.13.96.36 1.9.7 2.81a2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45c.91.34 1.85.57 2.81.7A2 2 0 0 1 22 16.92z"/></svg>
);
...
const IconDownload = ({ size = 14 }: { size?: number }) => (
  <svg viewBox="0 0 24 24" width={size} height={size} {...svgStroke} strokeWidth={2.5} aria-hidden="true">
...
const IconHome = ({ size = 14 }: { size?: number }) => (
  <svg viewBox="0 0 24 24" width={size} height={size} {...svgStroke} strokeWidth={2.2} aria-hidden="true">
    <path d="M3 9.5 12 3l9 6.5V20a1 1 0 0 1-1 1h-5v-6H9v6H4a1 1 0 0 1-1-1z"/>
  </svg>
);

const IconDocs = ({ size = 14 }: { size?: number }) => (
  <svg viewBox="0 0 24 24" width={size} height={size} {...svgStroke} aria-hidden="true">
    <path d="M4 4.5A2.5 2.5 0 0 1 6.5 2H19v18H6.5A2.5 2.5 0 0 0 4 22z"/>
    <path d="M8 7h7M8 11h7"/>
  </svg>
);

// Same button language as WelcomeActions: labelled pills for the two actions that
// aren't self-evident from a glyph, then the brand icon row. Icon-only would have
// made "Dokumentasi" (opens the publication gallery) undiscoverable.
function PersistentCTA({ onDocsClick }: { onDocsClick: () => void }) {
  return (
    <div className="ms-persistent-cta">
      <button
        className="ms-action-btn ms-action-btn--docs"
        onClick={onDocsClick}
      >
        <IconDocs />
      ...
      <div className="ms-welcome-social">
        <a className="ms-social-icon" href={mailtoUrl} onClick={openEmail}
           aria-label="Email" title="Email">
          <IconMail />
        </a>
        <a className="ms-social-icon ms-social-icon--in"
        ...
        <a className="ms-social-icon ms-social-icon--gh" href="https://github.com/Ebastari"
...
function WelcomeActions() {
  return (
    <div className="ms-welcome-actions">
      ...
      <div className="ms-welcome-social">
        <a className="ms-social-icon ms-social-icon--wa" href="https://wa.me/6281122220044"
        ...
        <a className="ms-social-icon" href="tel:+6281122220044"
           aria-label="Telepon" title="0811-2222-0044">
          <IconPhone />
        </a>
        <a className="ms-social-icon ms-social-icon--in"
           href="https://linkedin.com/in/agung-laksono-250524211"
